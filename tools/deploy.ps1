...
#   <repo>\release\  ->  %APPDATA%\Grasshopper\UserObjects\carcara\
...
#
# Run from the repo root after `python tools/build_userobjects.py` (or make_release.py):
...
# Mirror release\ -> UserObjects\carcara\ (excludes __pycache__).
robocopy $src $dst /MIR /XD __pycache__ /NFL /NDL /NJH /NJS /NP | Out-Null
if ($LASTEXITCODE -ge 8) { throw "robocopy failed copying release\ (exit $LASTEXITCODE)" }
